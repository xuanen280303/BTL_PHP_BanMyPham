<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" href="User/Images/logo1.jpg">
    <title>Hasaki.vn | Mỹ phẩm - Clinic & Spa</title>
    <link rel="stylesheet" href="User/css/support.css">
</head>
<body>
<header>
    <div class="header_support">
        <a href="index.html"><img src="User/Images/logo.svg" alt=""></a>
        <!-- <img src="User/Images/logo.svg" alt=""> -->
        <div class="taikhoan">
            <div class="one_line" style="margin-right: 5px;">Gửi yều cầu</div>
            <div class="one_line" style="margin-right: 5px;">|</div>
            <div class="one_line" style="margin-left: 3px;">
                <a href="logincustomer.html" class="dangnhap">Đăng nhập</a>
            </div>
        </div>
    </div>
</header>
        <div class="background">
            <div class="khungtimkiem">
                <h5>Xin chào! Chúng tôi có thể giúp gì được cho bạn?</h5>
                <div class="finds">
                    <input type="text" class="timkiem" placeholder="Nhập từ khoá để tìm sản phẩm, thương hiệu bạn mong muốn. Ví dụ: Hasaki">
                    <div class="icon_timkiems">
                        <img src="User/Images/icon_search1.svg" alt="">
                    </div> 
                </div> 
                <div class="info_lienhe">
                    <div class="icons">
                        <img src="User/Images/icon_dauhoi.svg" alt="">
                    </div>
                    <div class="phone">
                        <p>1800 6310</p>
                    </div>
                    <div class="others">
                        <p>(Khiếu nại, góp ý)</p>
                    </div>
                    <div class="icons">
                        <img src="User/Images/icon_block_call.svg" alt="">
                    </div>
                    <div class="phone">
                        <p>1800 6324</p>
                    </div>
                    <div class="others">
                        <p>(Tư vấn)</p>
                    </div>
                    <div class="icons">
                        <img src="User/Images/icon_chat1.svg" alt="">
                    </div>
                    <div class="phone">
                        <p>Chat</p>
                    </div>
                </div>                            
            </div>           
            <div class="support_icon">
                <div class="hinh1">
                    <div class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="41" height="42">
                            <path fill="#333" fill-rule="evenodd" d="M40.423 25.884l-.866.9-5.646-5.793.867-.898a1.947 1.947 0 0 1 2.806.016l2.825 2.895c.779.801.785 2.09.014 2.88zm-2.912 3.25l-5.891-6.043 1.347-1.297 5.892 6.044-1.348 1.296zm-4.69-2.959l1.541 1.584.01-.01 2.287 2.346-9.407 9.564a.799.799 0 0 1-.383.27l-6.708 2.021a.824.824 0 0 1-.824-.216.871.871 0 0 1-.21-.846l1.968-6.879c.05-.175.16-.31.288-.417l9.385-9.541 2.061 2.115-.008.009zm-7.488 11.717l-.122-2.215-2.217-.187-.453-.464-1.059 3.701.785.806 3.609-1.086-.543-.555zm-3.119-16.181c-.996 3.108.599 3.846.851 3.93.398.136.801.282 1.203.434l-5.041 5.124a4.085 4.085 0 0 0-1.179 1.9l-1.966 6.872c-.182.634-.182 1.286-.068 1.912C8.06 41.757.001 40.53.001 36.007v-1.32c0-3.521 6.188-7.089 11.464-9.046.381-.143 2.202-1.16 1.184-3.901-2.613-2.738-4.587-7.173-4.587-11.539C8.062 3.514 12.371.01 17.411.01c5.037 0 9.372 3.504 9.372 10.191 0 4.348-1.985 8.766-4.583 11.51h.014z"/>
                        </svg>
                    </div>
                    <div class="tittle-icon">
                        <h4>Tài khoản</h4>
                    </div>      
                </div>
                <div class="hinh1">
                    <div class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="41">
                            <path fill="#333" fill-rule="evenodd" d="M29.067 41H1.928c-.624 0-1.047-.67-.944-1.478l3.57-29.537c.062-.485.496-.874.967-.874h19.95c.473 0 .907.389.968.874l3.571 29.537c.102.808-.32 1.478-.943 1.478zM10.409 10.25c-.936 0-1.696.765-1.696 1.708s.76 1.709 1.696 1.709c.936 0 1.696-.766 1.696-1.709 0-.943-.76-1.708-1.696-1.708zm10.177 0c-.936 0-1.696.765-1.696 1.708s.76 1.709 1.696 1.709c.936 0 1.696-.766 1.696-1.709 0-.943-.76-1.708-1.696-1.708zm-.565-2.278c-.625 0-1.131-.509-1.131-1.139 0-2.954-.541-4.555-3.393-4.555-2.867 0-3.392 1.534-3.392 4.555 0 .63-.506 1.139-1.131 1.139s-1.13-.509-1.13-1.139c0-1.868 0-6.833 5.653-6.833 5.654 0 5.654 4.965 5.654 6.833 0 .63-.506 1.139-1.13 1.139z"/>
                        </svg>            
                    </div>
                    <div class="tittle-icon">
                        <h4>Đặt hàng</h4>
                    </div>       
                </div>
                <div class="hinh1">
                    <div class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="41" height="42">
                            <path fill="#333" fill-rule="evenodd" d="M23.654 41.993L0 30.233V8.805L19.605.976 41 8.805V30.233l-17.346 11.76zm-20.5-31.018v17.272l20.256 10.07.244-.165V18.988L3.245 10.939l-.091.036zm2.248-.897l19.195 7.57 4.851-2.597L11.04 7.826l-5.638 2.252zm14.253-5.692l-4.294 1.715 17.776 6.976 4.168-2.231-17.65-6.46zM7.884 26.216l11.039 5.269v1.972L7.884 28.188v-1.972zm9.462 2.707l-1.577-.608v-4.811l-1.592-.531 2.377-3.103 2.332 4.676-1.54-.515v4.892zm-6.308-2.741l-1.577-.608V21.2l-1.549-.517 2.335-3.019 2.376 4.592-1.585-.529v4.455z"/>
                        </svg>
                    </div>
                    <div class="tittle-icon">
                        <h4>Quy cách đóng gói</h4>
                    </div>       
                </div>
                <div class="hinh1">
                    <div class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="42" height="31">
                            <path fill="#333" fill-rule="evenodd" d="M40.384 26.346h-1.605c-.397-2.836-2.772-5.032-5.664-5.032-2.89 0-5.266 2.195-5.663 5.031-.885-.007-1.606-.741-1.606-1.645V8.237c0-.907.726-1.646 1.615-1.646h7.27c1.186 0 1.685.739 2.423 1.646l3.609 4.907S42 14.909 42 16.469V24.7c0 .906-.726 1.646-1.616 1.646zM38.667 15.07l-3.401-4.798s-.362-.389-.622-.389h-3.952v6.586h8.603c0-.412-.327-.948-.628-1.399zM22.615 26.346h-6.452c-.396-2.836-2.772-5.032-5.663-5.032-2.892 0-5.267 2.196-5.664 5.032H1.615C.726 26.346 0 25.606 0 24.7V1.652C0 .745.726.006 1.615.006h21c.89 0 1.616.739 1.616 1.646V24.7c0 .906-.726 1.646-1.616 1.646zM10.5 23.362c2.063 0 3.736 1.705 3.736 3.807 0 2.103-1.673 3.807-3.736 3.807s-3.736-1.704-3.736-3.807c0-2.102 1.673-3.807 3.736-3.807zm22.615 0c2.064 0 3.736 1.705 3.736 3.807 0 2.103-1.672 3.807-3.736 3.807-2.063 0-3.735-1.704-3.735-3.807 0-2.102 1.672-3.807 3.735-3.807z"/>
                        </svg>                   
                    </div>
                    <div class="tittle-icon">
                        <h4>Vận chuyển 2H</h4>
                    </div>       
                </div>
                <div class="hinh1">
                    <div class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="41" height="42">
                            <path fill="#333" fill-rule="evenodd" d="M23.654 41.993L0 30.233V8.805L19.605.976 41 8.805V30.233l-17.346 11.76zm-20.5-31.018v17.272l20.256 10.07.244-.165V18.988L3.245 10.939l-.091.036zm2.248-.897l19.195 7.57 4.851-2.597L11.04 7.826l-5.638 2.252zm14.253-5.692l-4.294 1.715 17.776 6.976 4.168-2.231-17.65-6.46zM7.884 26.216l11.039 5.269v1.972L7.884 28.188v-1.972zm9.462 2.707l-1.577-.608v-4.811l-1.592-.531 2.377-3.103 2.332 4.676-1.54-.515v4.892zm-6.308-2.741l-1.577-.608V21.2l-1.549-.517 2.335-3.019 2.376 4.592-1.585-.529v4.455z"/>
                        </svg>                        
                    </div>
                    <div class="tittle-icon">
                        <h4>Phí vận chuyển</h4>
                    </div>       
                </div>
                <div class="hinh1">
                    <div class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="36" height="41">
                            <path fill="#333" fill-rule="evenodd" d="M19.143 37.471V41L8.784 34.692l10.359-6.308v3.285c5.634-.768 9.976-5.469 9.976-11.169 0-3.132-1.312-5.966-3.427-8.015l4.361-3.874c3.071 3.073 4.971 7.262 4.971 11.889 0 8.883-6.978 16.174-15.881 16.971zM15.906 9.325c-5.634.77-9.977 5.474-9.977 11.175 0 3.135 1.312 5.968 3.43 8.016l-4.362 3.875C1.924 29.319.024 25.127.024 20.5c0-8.883 6.978-16.174 15.882-16.971V0l10.357 6.307-10.357 6.308v-3.29z"/>
                        </svg>
                    </div>
                    <div class="tittle-icon">
                        <h4>Đổi trả, hoàn tiền</h4>
                    </div>       
                </div>
                <div class="hinh1">
                    <div class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="35" height="46">
                            <path fill="#333" fill-rule="evenodd" d="M31.125 34.659c-.515 2.049-1.666 3.515-2.611 3.3a.85.85 0 0 1-.224-.088c-.062.126-.126.252-.198.395-.243.481-.502.934-.773 1.364-.028.046-.059.084-.088.129-.291.453-.595.875-.908 1.273-.107.136-.214.262-.323.391a15.74 15.74 0 0 1-.767.856c-.108.112-.214.226-.323.332-.061.06-.12.129-.182.187-.285.27-.571.519-.857.752l-.021.019-.005.004c-.04.032-.079.059-.118.09-.241.193-.481.375-.718.541-.096.068-.189.126-.284.19-.178.119-.355.236-.528.341-.103.062-.203.118-.304.176-.159.09-.316.177-.468.256a10.85 10.85 0 0 1-.719.338c-.09.038-.178.076-.264.109a8.08 8.08 0 0 1-.387.139c-.075.025-.151.051-.222.072a5.115 5.115 0 0 1-.355.089c-.053.011-.11.027-.159.036a2.554 2.554 0 0 1-.407.042h-.001l-.048-.001c-.048-.002-.113-.015-.167-.02-.072-.008-.138-.012-.22-.027-.071-.013-.155-.036-.233-.055-.091-.022-.177-.04-.278-.069-.088-.027-.186-.064-.281-.097-.105-.036-.206-.067-.319-.112-.105-.041-.219-.095-.33-.143-.115-.05-.225-.094-.345-.152-.122-.058-.252-.131-.378-.197-.12-.063-.235-.119-.359-.189-.137-.078-.281-.171-.423-.258-.12-.074-.237-.141-.359-.222-.154-.101-.312-.22-.469-.332-.115-.083-.228-.157-.344-.245-.188-.143-.378-.305-.567-.464-.087-.073-.173-.136-.26-.213a15.937 15.937 0 0 1-1.096-1.062l-.013-.012c-.313-.334-.619-.7-.92-1.083-.086-.11-.169-.226-.254-.341a16.092 16.092 0 0 1-.664-.962c-.084-.131-.168-.26-.25-.396-.28-.465-.55-.949-.798-1.47a.872.872 0 0 1-.227.089c-.762.174-1.659-.747-2.248-2.187-2.977-2.328-4.94-6.07-6.013-11.744-.271-1.431-.036-3.548.707-4.545 1.964-2.632 5.672-5.886 11.776-7.542a30.88 30.88 0 0 1 5.38-.939c1.264-.111 2.6-.159 4.016-.132a36.884 36.884 0 0 0-4.016-.354c-1.973-.06-3.762.064-5.38.334C5.766 12.144 1.947 16.771.617 20.226-2.015 8.959 4.014 1.866 13.531.321a26.29 26.29 0 0 1 5.38-.292c.201.009.402.02.604.033 1.584.102 3.033.403 4.335.854 5.271 1.828 8.119 6.128 7.627 9.8 2.275 1.305 6.804 16.162-.352 23.943zm-16.133 4.425c.026.079.045.156.089.24.031.06.083.124.123.186.051.079.094.156.157.236.054.067.125.134.187.201.07.076.132.153.212.228.073.069.162.133.242.2.085.07.164.143.256.21.09.065.191.121.287.182.097.062.189.128.293.184.101.056.211.098.316.148.109.05.214.107.326.15.109.041.223.066.334.099.118.036.233.078.354.103.112.024.226.027.339.041.132.016.263.042.396.044.118.002.233-.024.349-.036.13-.013.263-.013.391-.041.095-.021.185-.061.278-.089.142-.042.286-.078.422-.135.07-.03.133-.071.202-.104.151-.073.304-.144.445-.23.043-.026.079-.058.121-.086.158-.103.314-.208.454-.321.018-.015.032-.03.049-.045.158-.13.307-.263.437-.398l.007-.008c.142-.146.267-.293.371-.433.006-.007.008-.013.013-.02.099-.135.181-.265.24-.383.061-.124.098-.235.108-.328-.556.472-2.242.422-3.091.753a1.61 1.61 0 0 1-.385.09c-.049.006-.1.004-.15.007-.084.006-.168.015-.253.013l-.023-.002-.025.001c-.107-.003-.204-.017-.303-.028-.048-.006-.1-.007-.145-.014a2.087 2.087 0 0 1-.356-.082c-.92-.329-2.497-.08-3.122-.682.009.047.039.1.055.149zm-.068-.24c.588.338 1.449.358 2.271.365.51.005 1.007.005 1.406.071.054.009.143.013.236.016l.052.001.117-.001.084-.002c.064-.003.122-.007.161-.014.737-.125 2.87-.111 3.538-.46-.006-.074-.023-.092-.067-.107-.447-.163-2.134-.714-2.919-.761a.989.989 0 0 0-.274.014 2.813 2.813 0 0 1-.618.09l-.023-.003-.025.003a2.341 2.341 0 0 1-.59-.09c-.485-.131-2.273.502-2.988.718-.13.039-.237.069-.28.073-.058.007-.083.038-.082.077l.001.01zm10.421-22.915c2.444-.34 3.81-1.805 4.645-3.492-1.315 1.182-3.548 1.776-6.14 2.299-1.557.314-3.243.603-4.939.977-1.833.405-3.677.912-5.38 1.66-.808.356-1.585.765-2.314 1.245-4.294 2.831-6.67 8.68-4.626 15.559-.146-.758-.181-1.477-.117-2.091.709 2.31 1.723 3.728 2.612 4.931-.626-1.78-1.779-3.979-2.319-8.455a.63.63 0 0 0-.036-.2c.393-3.194 1.661-6.534 5.085-8.659a19.218 19.218 0 0 1 1.715-.95 16.44 16.44 0 0 1 1.444-.621c.151-.057.294-.093.442-.144.311-.106.623-.216.923-.299.175-.048.342-.079.514-.121.263-.063.53-.133.787-.183.192-.038.378-.063.568-.095.218-.036.439-.078.654-.108a.918.918 0 0 0 .048-.007c.449-.063.889-.112 1.327-.161.552-.062 1.099-.123 1.647-.196.225.654.524 1.616.562 2.726.063 1.85-.599 4.109-3.535 6.031.358-.052.691-.119 1.013-.193.095-.022.186-.046.278-.07.23-.059.451-.124.662-.194.083-.027.169-.054.249-.083.273-.099.535-.204.777-.32l.037-.019a7.32 7.32 0 0 0 .646-.358c.066-.04.128-.082.191-.124.151-.1.296-.204.433-.312.054-.042.109-.083.161-.126.177-.148.345-.301.498-.459.044-.046.081-.095.123-.142a6.24 6.24 0 0 0 .341-.397c.054-.07.099-.143.15-.214.083-.117.167-.233.241-.352.021-.033.037-.068.057-.101.133-.223.251-.45.358-.678.029-.064.059-.127.087-.191a9.953 9.953 0 0 0 .313-.814c.082-.245.152-.488.215-.728l.051-.193c.061-.242.118-.482.168-.712 3.287 3.264 4.443 7.26 4.903 10.308a31.48 31.48 0 0 1-.198 1.375c-.044.265-.091.504-.138.746-.027.141-.054.286-.082.421a20.661 20.661 0 0 1-.264 1.118c-.046.174-.092.35-.139.512-.36 1.27-.753 2.264-1.091 3.113-.08.201-.153.378-.224.543.896-1.215 1.906-2.659 2.601-5.015.054.393.066.835.031 1.308.529-2.847-.147-12.288-6.015-17.295z"/>
                        </svg>                        
                    </div>
                    <div class="tittle-icon">
                        <h4>Dịch vụ SPA</h4>
                    </div>       
                </div>
                <div class="hinh1">
                    <div class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="40" height="41">
                            <path fill="#333" fill-rule="evenodd" d="M39.842 16.562L29.101 40.057c-.259.568-.828.926-1.45.926-.043 0-.087-.001-.13-.004l-.316-.026c-9.766-.77-22.206-2.818-26.007-8.201-1.159-1.643-1.486-3.546-.949-5.504l.004-.013.007-.032c.002-.006.007-.01.008-.017.003-.007.009-.013.011-.021.029-.091.058-.182.104-.268L12.154 4.492c.304-.581.937-.907 1.605-.835.104.011 10.627 1.216 19.087-3.44.592-.33 1.337-.244 1.842.21.506.454.662 1.177.39 1.796L32.57 7.917c.604.157 1.678.339 3.435.339.547 0 1.055.279 1.348.737.29.459.323 1.033.087 1.522l-1.71 3.521c.71.16 1.614.301 2.654.301h.008c.541 0 1.045.271 1.337.721.293.45.335 1.016.113 1.504zM14.485 6.884L4.21 26.44c4.983.742 13.762 1.391 17.264-1.209l9.024-20.494c-6.68 2.462-13.401 2.322-16.013 2.147zm16.804 3.943l-7.052 16.015c-.095.215-.237.406-.416.56-2.591 2.228-7.099 2.863-11.402 2.863-2.3 0-4.539-.182-6.398-.399 3.948 1.695 11.531 2.333 17.431 2.187l10.084-20.772c-.991-.11-1.713-.281-2.247-.454zm3.034 6.109l-8.42 17.345c-.255.525-.783.868-1.371.891-.627.024-1.662.055-2.96.055-3.676 0-9.456-.25-14.086-1.549 4.012 1.917 10.635 3.343 19.179 4.068L36.012 17.3c-.652-.102-1.218-.234-1.689-.364zm-24.084 3.339s1.201-2.162 1.845-3.316c.753-1.353.932-1.527 2.413-2.108.566-.222 1.796-.196 2.044-.843.17-.45.427-.699.198-.873-.391-.297-.725-1.155-.483-1.799.398-1.058.903-2.554 3.066-2.628 1.966-.067 3.066.922 2.337 2.905-.497 1.355-1.019 1.601-1.392 1.847-.398.263-.398.32-.572.886-.171.557 1.078.616 1.691.616 1.218 0 1.481.578 1.069 1.92l-.919 1.454c-3.065 2.782-8.709 2.125-11.297 1.939zm.945 2.568c-.095.478.151.955.724.729.485-.189.661-.82.968-1.19.247-.296.937-.433 1.303-.256.519.25-.15.926.323 1.172.595.311.951-.475 1.179-.869.246-.423.584-.47 1.026-.546.256-.047.833-.237 1.084-.133.293.123-.062.474.35.56.309.063.491-.143.675-.372.206-.26.523-.377.815-.509.434-.197.872-.36 1.282-.608l-1.221 2.459c-.216.435-.252.544-.704.777-.314.161-.606.247-.96.133-.409-.13-.417-.558-.756-.752-.396-.225-.851.437-1.08.708-.313.375-.571.888-1.125.914-.228.011-.428.007-.587-.166-.151-.168-.104-.422-.275-.576-.367-.329-.914-.118-1.181.216-.258.324-.493.795-1.002.632-.541-.171-.251-.742-.62-1.016-.303-.224-.965-.021-1.236.196-.328.265-.561.698-1.024.737-.121.011-1.108-.202-1.133-.151l.961-1.916c.169-.338.317-.704.514-1.026.284-.466 1.111-.131 1.593-.08.576.062.177.585.107.933z"/>
                        </svg>
                    </div>
                    <div class="tittle-icon">
                        <h4>Tuyển dụng</h4>
                    </div>       
                </div>
            </div>
        </div>  
    <div class="cover_support">       
        <div class="phanthan-tren">
            <div class="support_tittle">
                Câu hỏi thường gặp <br>             
            </div>
            <div class="support_questions">
                <div class="questions_left">
                    <ul>
                        <li>Đăng ký thành viên Hasaki như thế nào?</li>
                        <li>Tại sao tôi không thể đăng nhập vào tài khoản của tôi?</li>
                        <li>Tôi có thể sử dụng chung tài khoản với người khác không?</li>
                    </ul>
                </div>
                <div class="questions_right">
                    <ul>
                        <li>Có cần đặt lịch trước khi đến Spa hay không?</li>
                        <li>Đặt dịch vụ như thế nào?</li>
                        <li>Khám da tại Spa có tốn phí hay không?</li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="phanthan-duoi">
            <div class="support_tittle">
                Thông tin hỗ trợ <br>             
            </div>
            <div class="support_questions">
                <div class="cot-1">
                    <p>Giới thiệu Hasaki</p> <br>
                    <p>Hướng dẫn đặt hàng 2H</p> <br>
                    <p>Chính sách vận chuyển giao nhận</p> <br>                  
                </div>
                <div class="cot-2">
                    <p>Khách hàng thân thiết</p> <br>
                    <p>Hướng dẫn thanh toán trực tuyến</p> <br>
                    <p>Điều khoản sử dụng</p> <br>                  
                </div>
                <div class="cot-3">
                    <p>Hướng dẫn đổi quà</p> <br>
                    <p>Thẻ quà tặng Got It</p> <br>
                    <p>Chính sách bảo mật</p> <br>                  
                </div>
                <div class="cot-4">
                    <p>Hướng dẫn đặt hàng</p> <br>
                    <p>Phiếu mua hàng</p> <br>
                    <p>Hướng dẫn tải & sử dụng App Hasaki</p> <br>                  
                </div>
            </div>
        </div>
    </div> 
    <section class="footer">
        <div class="footer-in">
            <div class="footer1">
                <div class="anh_footer">
                    <img src="User/Images/banner_footer_sp.jpg" alt="">
                </div>               
                <a href="https://www.facebook.com/Hasaki.vn/"></a>
            </div>
            <div class="footer2">
                <div class="tieude1">
                    Hỗ Trợ khách Hàng
                </div>
                <div class="content0">Hotline: 1800 6324</div>
                <div class="content0">Mỹ phẩm: 8:00-22:00</div>
                <div class="content0">Clinic & Spa: 9:00-20:00</div>
                <div class="content2">Các câu hỏi thường gặp</div>
                <div class="content2">Gửi yêu cầu hỗ trợ</div>
                <div class="content2">Hướng dẫn đặt hàng</div>
                <div class="content2">Phương thức vận chuyển</div>
                <div class="content2">Chính sách đổi trả</div>
            </div>
            <div class="footer3">
                <div class="tieude1">
                    Về hasaki.vn
                </div>
                <div class="content2">Phiếu mua hàng</div>
                <div class="content2">Giới thiệu Hasaki.vn</div>
                <div class="content2">Tuyển dụng</div>
                <div class="content2">Chính sách bảo mật</div>
                <div class="content2">Điều khoản sử dụng</div>
                <div class="content2">Liên hệ</div>
                <div class="content2">Vị trí cửa hàng</div>
            </div>
            <div class="footer4">
                <div class="tieude1">
                    Hợp tác & Liên kết
                </div>
                <div class="content2">Hasaki Clinic & Spa</div>
                <div class="content2">Hasaki cẩm nang</div>
                <div class="tieude1" style="margin-top: 20px;">
                    Tải ứng dụng
                </div>
                <div class="footer4-hinhanh">
                    <img src="User/Images/qr_download_app.svg" style="width: 100px;" alt="">
                    <div class="footer4-hinhanh_anhphu">
                        <div>                           
                            <img src="User/Images/img_app_store.svg" style="width: 100px;" alt="">
                        </div>
                        <img src="User/Images/img_google_play.svg" style="width: 100px;" alt="">
                    </div>
                </div>
            </div>
        </div>
    </section>
    <div class="support_lienhe">
        <div>
            <img src="User/Images/icon_map.jpg" alt="">
            <p class="help" style="width: 60px ; top: 37%; left: -50px;">Bản đồ</p>
        </div>
        <div>
            <img src="User/Images/icon_contact.jpg" alt="">
            <p class="help" style="width: 210px; top: 37%; left: -200px;">Để lại lời nhắn cho chúng tôi</p>
        </div>
        <div>
            <img src="User/Images/icon_call.jpg" alt="">
            <p class="help" style="width: 70px; top: 37%; left: -60px;">Gọi ngay</p>
        </div>
        <div>
            <img src="User/Images/icon_messenger.jpg" alt="">
            <p class="help" style="width: 160px; top: 37%; left: -150px;">Facebook Messenger</p>
        </div>      
    </div>
</body>
</html>